...
from app.constants.error import (GROUP_NOT_FOUND_404, ATTENDANCE_NOT_FOUND_404,
                                 ...
                                 USER_ALREADY_TAKE_ATTENDANCE_500, USER_NOT_GROUP_CREATOR_301)
...
class AttendanceController:
    ...
    def get_attendance_info(self, event_id):
        ...
        result = list(map(lambda x: {
            ...
            "fb_id": str(User.query.filter(User.id == x.user_id).first().fb_id)
        }, members_indicate))
    ...
    def get_group_attendance_csv(self, user, group_id):
        logging.info("Getting CSV attendance for group {group_id}".format(group_id=group_id))
        ...
        all_events = Event.query.filter(Event.group_id == group_id,
                                        Event.is_deleted == False).all()
        all_events = sorted(all_events, key=lambda x: x.start_date)
        ...
        for member in all_members:
            ...
            # Check attendance
            for event in all_events:
                attendance = Attendance.query.filter(Attendance.user_id == member.id,
                                                     Attendance.event_id == event.id).first()
                if not attendance:
                    result.append(",0")
                    continue
